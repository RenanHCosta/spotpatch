...
Execution: exige investigação executável, risco aceitável e ausência de run ativo. O executor é iniciado automaticamente, cria branch/commit/PR via Deco GitHub Connection e o SAVE valida base, branch, provider, arquivos e PR antes de mover para `pull_request_opened`. Nunca há merge automático.
