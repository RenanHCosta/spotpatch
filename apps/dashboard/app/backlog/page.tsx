...
function FeedbackPanel({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  const action = useMutation({
    ...
    onMutate: async (name) => {
      ...
      client.setQueryData<Item[]>(["feedback"], (current) =>
        current?.map((item) => (item.id === id ? { ...item, status: "investigating" } : item)),
      );
  ...
  const deleteFeedback = useMutation({
    mutationFn: () => api<{ id: string }>("/api/admin/feedback/" + id, { method: "DELETE" }),
    onSuccess: async () => {
      client.setQueryData<Item[]>(["feedback"], (current) =>
    ...
    >
      ...
      <div className="flex h-10 shrink-0 items-center border-b border-line px-3">
        <button
          type="button"
          onClick={onClose}
          className="mr-2 grid size-7 place-items-center rounded-[4px] text-mute hover:bg-canvas min-[640px]:hidden"
          aria-label="Voltar"
        >
          <ChevronLeft size={16} />
        </button>
        ...
        <span className="ml-2 hidden font-mono text-[10px] text-mute-soft min-[640px]:inline">
          {Math.round(width)}px
        </span>
        {detail && (
          <button
            ...
            onClick={() => {
              if (
                window.confirm(
                  `Excluir o feedback #${String(detail.public_number).padStart(2, "0")} do board?`,
                )
              )
      ...
      {query.isPending ? (
        ...
        <>
          <div className="min-h-0 flex-1 overflow-y-auto">
            ...
            <dl className="divide-y divide-line border-b border-line">
              {[
                ...
                <div key={label} className="grid grid-cols-[72px_1fr] gap-2 px-3 py-2">
                  <dt className="text-[10px] font-semibold uppercase tracking-[0.08em] text-mute">
                    {label}
                  </dt>
            ...
            <section className="border-b border-line p-3">
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.08em] text-mute">
                Capturas
              </p>
              <div className="space-y-3">
                <Screenshot
                  label="Viewport"
                  url={screenshots.data?.viewportUrl}
                  loading={screenshots.isLoading}
                  width={detail.viewport.width}
                  height={detail.viewport.height}
                />
                <Screenshot
                  label="Elemento"
                  url={screenshots.data?.elementUrl}
                  loading={screenshots.isLoading}
                  width={Math.max(1, Math.round(detail.element.boundingBox.width))}
                  height={Math.max(1, Math.round(detail.element.boundingBox.height))}
                />
              </div>
              {screenshots.error && (
                <button
                  type="button"
                  onClick={() => screenshots.refetch()}
                  className="mt-2 text-[11.5px] text-danger underline"
                >
            ...
            <section className="p-3">
              <p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.08em] text-mute">
                Timeline
              </p>
                ...
                {detail.events.map((event) => (
                  <li key={event.id} className="relative pl-4">
                    ...
                    <p className="mt-0.5 font-mono text-[10px] text-mute">
                      {formatTime(event.created_at)} · {event.actor_label}
                    </p>
                  </li>
                ))}
              </ol>
          ...
          <div className="flex shrink-0 flex-wrap gap-2 border-t border-line bg-surface p-3">
            {canStartInvestigation(detail) && (
              <button
                type="button"
                disabled={action.isPending}
                onClick={() => action.mutate("investigate")}
                className="h-9 flex-1 rounded-[4px] bg-accent px-3 font-semibold text-surface hover:bg-accent-hover disabled:opacity-50"
              >
                Investigar
              </button>
            )}
            {detail.status === "pull_request_opened" && detail.execution?.previewUrl && (
              <a
                href={detail.execution.previewUrl}
                target="_blank"
                rel="noreferrer"
                className="inline-flex h-9 flex-1 items-center justify-center gap-1 rounded-[4px] border border-line bg-surface px-3 font-semibold text-ink hover:bg-canvas"
              >
                Ver preview <ExternalLink size={14} />
              </a>
            )}
            {detail.status === "pull_request_opened" && (
              <button
                type="button"
            ...
            {detail.status === "pull_request_opened" && detail.execution?.pullRequestUrl && (
              <a
                href={detail.execution.pullRequestUrl}
                target="_blank"
                rel="noreferrer"
                className="inline-flex h-9 flex-1 items-center justify-center gap-1 rounded-[4px] border border-line bg-surface px-3 font-semibold text-ink hover:bg-canvas"
              >
            ...
            {!["completed", "rejected", "pull_request_opened"].includes(detail.status) && (
              <button
                type="button"
                disabled={action.isPending}
                onClick={() => action.mutate("reject")}
                className="h-9 rounded-[4px] border border-line bg-surface px-3 font-medium text-ink hover:bg-canvas disabled:opacity-50"
              >
                Rejeitar
              </button>
            )}
    ...
    <div>
      ...
      {loading ? (
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="block border border-line bg-canvas"
        >
          <Image
            src={url}
            alt={"Captura: " + label.toLowerCase()}
        ...
        <div className="grid aspect-video place-items-center border border-line bg-canvas font-mono text-[11px] text-mute-soft">
...
export default function Backlog() {
  ...
  useEffect(() => {
    ...
    setProjectFilter(
      params.get("project") ?? localStorage.getItem("spotpatch_current_project") ?? "all",
    );
  ...
  const last24Hours = useMemo(
    () =>
      projectRows.filter((item) => Date.now() - new Date(item.created_at).getTime() <= 86400000)
        .length,
    [projectRows],
  );
  const rows = useMemo(() => {
      ...
      <div className="flex h-[calc(100vh-104px)] min-h-[520px] overflow-hidden">
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="flex h-9 shrink-0 items-center border-b border-line bg-surface px-3">
            <div
              className="flex h-7 rounded-[4px] border border-line p-px"
            ...
            <a
              href="/demo"
              className="ml-3 inline-flex items-center gap-1 font-medium text-accent underline-offset-2 hover:underline"
            >
              <Plus size={14} />
              Novo
            </a>
                    ...
                    <tbody className="divide-y divide-line">
                      {rows.map((item) => (
                        ...
                        >
                          ...
                          <td className="px-2 font-mono text-[11px] text-mute-soft">
                    ...
                    {rows.map((item) => (
                      ...
                      >
                        ...
                        <span className="mt-1 block font-mono text-[11px] text-mute">
                          #{String(item.public_number).padStart(2, "0")} ·{" "}
                          {formatTime(item.created_at)} · {item.project.name}
                        </span>
                      </button>
                    ))}
                  </div>
  ...
  return (
    ...
    >
      <button type="button" onClick={onOpen} className="block w-full text-left">
        ...
        <span className="mt-1 line-clamp-2 text-[12.5px] font-medium leading-snug">
          {item.title}
        </span>
